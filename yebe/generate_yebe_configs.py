    ...
    def fetch_multiple_countries(self, country_codes: List[str]) -> List[Dict]:
        ...
        for code in country_codes:
            ...
            if result["status"] == "success":
                print(
                    f"✅ {result['flag_emoji']} {result['country_name']} - {result['content_length']} characters"
                )
            else:
                print(
                    f"❌ {result['flag_emoji']} {result['country_name']} - Error: {result.get('error', 'Unknown error')}"
                )
        return results
        ...
        for result in results:
            if result["status"] == "success" and result["content"]:
                ...
                with open(filename, "w", encoding="utf-8") as f:
                    f.write(
                        f"Country: {result['flag_emoji']} {result['country_name']} ({result['country_code']})\n"
                    )
...
if __name__ == "__main__":
    ...
    if results:
        ...
        print(
            f"Successful fetches: {sum(1 for r in results if r['status'] == 'success')}"
        )
        ...
        if successful:
            ...
            for r in successful:
                if r["country_name"] == "Fastest Location":
                    r["flag_emoji"] = "⚡️"
                config = build_config(
        ...
        if failed:
            ...
            for r in failed:
                print(
                    f"   {r['flag_emoji']} {r['country_name']} ({r['country_code']}) - {r.get('error', 'Unknown error')}"
                )
    else:
        print(
